...
# ---------------------------------------------------------------------------
# Delegate to official LoRA-GA training script
# ---------------------------------------------------------------------------
GPU_IDS="${1:-0}"
RUN_EVAL="${2:-false}"
...
CONFIG_PATH="${REPO_ROOT}/configs/nlg/metamath/lora_ga.yaml"

echo "[INFO] Delegating to official LoRA-GA repo: ${LORA_GA_PATH}"
echo "[INFO] Config: ${CONFIG_PATH}"
echo "[INFO] GPU IDs: ${GPU_IDS}"
echo "[INFO] Eval after train: ${RUN_EVAL}"
...
if [[ -f "${LORA_GA_PATH}/train.py" ]]; then
    ...
    echo "[ERROR] Expected: ${LORA_GA_PATH}/train.py or ${LORA_GA_PATH}/run_lora_ga.py"
