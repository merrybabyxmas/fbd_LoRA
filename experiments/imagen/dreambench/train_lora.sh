#!/usr/bin/env bash
# Train standard LoRA baseline on a DreamBench concept.
# Usage: SEED=42 CONCEPT_NAME=dog bash experiments/imagen/dreambench/train_lora.sh [config]
set -euo pipefail
...
echo "[train_lora.sh] CONFIG=${CONFIG} SEED=${SEED} CONCEPT=${CONCEPT_NAME}"
