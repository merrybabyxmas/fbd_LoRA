#!/usr/bin/env bash
# Train FBD-LoRA on a CustomConcept101 concept.
# Usage: SEED=42 CONCEPT_NAME=cat_statue bash experiments/imagen/customconcept101/train_fbd.sh [config]
set -euo pipefail
...
CONFIG=${1:-configs/imagen/customconcept101_fbd.yaml}
SEED=${SEED:-42}
...
echo "[customconcept101/train_fbd.sh] CONFIG=${CONFIG} SEED=${SEED} CONCEPT=${CONCEPT_NAME}"
