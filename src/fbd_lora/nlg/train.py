...
Usage:
    python -m fbd_lora.nlg.train --config configs/nlg/metamath_fbd.yaml --seed 42
...
def main() -> None:
    ...
    adapter_name = OmegaConf.select(cfg, "adapter.name", default="fbd")
    ...
    lora_config = LoraConfig(
        r=rank,
        lora_alpha=alpha,
        target_modules=target_mods if target_mods else None,
    ...
    # Load dataset
    from fbd_lora.nlg.data import load_metamathqa, tokenize_dataset

    dataset_path = OmegaConf.select(cfg, "dataset.hf_path", default="meta-math/MetaMathQA")
    ...
    prompt_template = OmegaConf.select(cfg, "dataset.prompt_template", default="alpaca_math")

    if is_smoke:
        max_samples = int(OmegaConf.select(cfg, "dataset.max_samples", default=32))
        max_seq_length = min(max_seq_length, 256)
    ...
    if is_smoke:
        max_steps = int(OmegaConf.select(cfg, "training.max_steps", default=5))
        num_epochs = 1
    ...
    ckpt_callback = FBDCheckpointCallback(
        ...
        fbd_config=fbd_cfg if fbd_state else None,
